...
const mutations = [
  ...
  {
    name:'§66 「電話を切る」の実行属性を外す', file:'p4_view.js',
    from:'{ attrs:\'data-end-call="1"\', body:\'<span class="opt-label">電話を切る</span>\' }',
    to:'{ attrs:\'data-tell="hangup"\', body:\'<span class="opt-label">電話を切る</span>\' }',
    expected:'「伝える」の項目から data-end-call="1" が欠けている',
  },
  {
  ...
  {
    name:'§66 終話時に確認画面を戻す', file:'p5_events.js',
    from:'    if (t) endCurrentCall(t);',
    to:"    if (t){ state.ui = defaultUi('hangup_confirm'); render(); }",
    expected:'「伝える」の終話が確認なしの即時処理へ接続されない',
  },
  {
  ...
  {
    name:'§66 「電話を切る」を原因説明の後ろへ動かす', file:'p4_view.js',
    ...
    to:"    { attrs:'data-tell=\"close\"', body:'<span class=\"opt-label\">' + (t.symptomResolved ? '原因を伝える' : '原因と対処を伝える') + '<span class=\"opt-sub\">' + (t.symptomResolved ? '復旧した原因をご説明します。' : '原因を見立てて、対処をご案内します。') + '</span></span>' },\n    { attrs:'data-end-call=\"1\"', body:'<span class=\"opt-label\">電話を切る</span>' },",
    expected:'「伝える」のID・項目名・注意書きが完全一致しない',
  },
  {
    name:'§66 終話の挨拶を別文言へ変える', file:'p2_data.js',
    from:"  interrupt:'失礼します。',",
    to:"  interrupt:'電話を切ります。',",
    expected:'折り返し約束の終話で「失礼します」が会話ログに残らない',
  },
  {
    name:'§66 通常切断を二段階のままに戻す', file:'p3_game.js',
    from:'  t.pendingInterruption = true;\n  finishInterruptedCall(t);\n  /* 先に回線を切って再入電へ移す。苛立ち加算を先に行うと100到達時だけ\n     顧客側の怒り終話画面を挟み、§66の即時終話ではなくなる。 */\n  addStress(t, REDIAL_STRESS);',
    to:'  if (!addStress(t, REDIAL_STRESS)){ render(); return; }\n  t.pendingInterruption = true;\n  state.ui = defaultUi();\n  render();',
    expected:'通常切断が確認画面や怒り終話を挟まず再入電へ進まない',
  },
  {
  ...
  {
    name:'§66 撤去した折り返し終話案内データを戻す', file:'p2_data.js',
    from:"    note:'折り返しをお約束しました。切る前に、折り返し先を確認できます。',",
    to:"    note:'折り返しをお約束しました。切る前に、折り返し先を確認できます。',\n    guide:'折り返しをお約束しました。「電話を切る」で、こちらから掛け直します。',",
    expected:'§66 検査: 廃止した折り返し終話案内データが残っている',
  },
  {
  ...
  {
    name:'§66 撤去したシフト帯をDOMへ戻す', file:'p1_head.html',
    from:'  </div>\n</div>\n\n<section class="office-scene"',
    to:'  </div>\n  <div id="topbar-shift-slot"><div class="shift-strip" id="shift-strip"></div></div>\n</div>\n\n<section class="office-scene"',
    expected:'§66 検査9: 撤去したシフト帯が残っている: shift-strip',
  },
  {
    name:'§66 小画面で時計を隠す', file:'p1_head.html',
    from:'.topbar-inner > .clock{ display:flex; grid-row:2; grid-column:1 / -1; justify-self:center; }',
    to:'.topbar-inner > .clock{ display:none; }',
    expected:'§66 検査10: 小画面で時計が消える',
  },
  {
  ...
  {
    name:'§66 小画面でオフィス操作を固定へ戻す', file:'p1_head.html', layout:true,
    from:'.office-call-actions{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px; }',
    to:'.office-call-actions{ position:fixed; left:0; right:0; bottom:0; display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px; }',
    expected:'オフィス操作が固定配置のまま',
  },
  {
    name:'§66 小画面で直近の顧客発話を消す', file:'p1_head.html', layout:true,
    from:'body.playing .transcript.recent{ min-height:150px; max-height:none; overflow:visible; }',
    to:'body.playing .transcript.recent{ display:none; min-height:150px; max-height:none; overflow:visible; }',
    expected:'通話の縦一列 の要素が描画されていない',
  },
  {
  ...
  {
    name:'§66 小画面で満足度メーターをstickyへ戻す', file:'p1_head.html', layout:true,
    from:'body.playing .stress-panel{ position:static; top:auto; z-index:auto; margin:12px; padding:15px; }',
    to:'body.playing .stress-panel{ position:sticky; top:0; z-index:23; margin:12px; padding:15px; }',
    expected:'通話要素が通常フローから外れている',
  },
  {
    name:'§66 小画面で質問選択肢の高さを0にする', file:'p1_head.html', layout:true,
    from:'body.playing .opts{ max-height:none; overflow:visible; }',
    to:'body.playing .opts{ height:0; min-height:0; max-height:0; overflow:hidden; }',
    expected:'「伝える」の選択肢が高さ0に潰れている',
  },
];
